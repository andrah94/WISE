// 21st.dev: ibelick/spotlight (tilt registry dependency)
...
export function Spotlight({ className, size = 200, springOptions = { bounce: 0 } }: SpotlightProps) {
  ...
  const handleMouseMove = useCallback((event: MouseEvent) => {
    ...
    mouseX.set(event.clientX - left); mouseY.set(event.clientY - top)
  ...
  return (
    <motion.div ref={containerRef}
      className={cn('pointer-events-none absolute rounded-full bg-[radial-gradient(circle_at_center,var(--tw-gradient-stops),transparent_80%)] blur-xl transition-opacity duration-200', 'from-zinc-50 via-zinc-100 to-zinc-200', isHovered ? 'opacity-100' : 'opacity-0', className)}
